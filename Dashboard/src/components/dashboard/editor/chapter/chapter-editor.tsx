...
import { QuillNovelEditor } from '@/components/dashboard/editor/writing/quill-novel-editor';
...
<p>Ready to begin your story? Just start typing below...</p>`;
  ...
  const handleSave = async (snapshot?: any, showNotification = true) => {
    setIsSaving(true);
    try {
      const effectiveSnapshot = snapshot ?? contentSnapshotRef.current;
      const contentToSave = effectiveSnapshot?.html || content;
      ...
      const updatedChapter: Chapter = {
        ...
        contentDelta: effectiveSnapshot?.delta ?? null,
        images: effectiveSnapshot?.images ?? [],
        wordCount,
  ...
  const handleContentChange = (snapshot: any) => {
  ...
  const handleAutoSave = (snapshot: any) => {
    return handleSave(snapshot, false);
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <div className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        {/* Quill Novel Editor - Take calculated height */}
