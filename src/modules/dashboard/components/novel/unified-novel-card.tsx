...
export function UnifiedNovelCard({ 
  variant, 
  data, 
  onClick, 
  onAction, 
  className 
}: UnifiedNovelCardProps) {
      ...
      <Card className={cn("overflow-hidden", className)}>
        <div className="flex h-full">
          <div className="relative w-1/3">
            <Image src={data.cover || "/placeholder.svg"} alt={data.title} fill className="object-cover" />
          </div>
          ...
          <div className="relative">
            <div className="aspect-[3/4] w-full rounded-t-xl overflow-hidden">
              <div
                ...
                style={{ backgroundImage: `url(${data.cover || `https://placehold.co/400x600/1a1a1a/ffffff?text=${encodeURIComponent(data.title || 'Novel')}`})` }}
              />
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <IconText icon={Eye}>{formatCount(data.views)}</IconText>
                <IconText icon={Heart}>{formatCount(data.collections)}</IconText>
              ...
              <div className="flex items-center justify-between text-xs text-muted-foreground">
                ...
                <span>{formatCount(data.words)} words</span>
              </div>
        ...
        <CardContent className="p-0">
          <div className="relative">
            <div className="aspect-[4/5] w-full rounded-t-xl overflow-hidden">
              <div
                className="absolute inset-0 bg-cover bg-center transition-transform duration-300 group-hover:scale-105"
                style={{ backgroundImage: `url(${data.cover || `https://placehold.co/300x400/1a1a1a/ffffff?text=${encodeURIComponent(data.title || 'Novel')}`})` }}
              />
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
              <div className="absolute top-3 left-3">
                <StatusBadge status={data.status} variant="overlay" />
            ...
            <div className="p-4 space-y-3">
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <IconText icon={Eye} size="xs">{formatCount(data.views)}</IconText>
              </div>
        ...
        <CardContent className="p-0">
          <div className="flex items-center gap-4 p-6">
            <div className="relative w-10 h-12 rounded-lg overflow-hidden">
              <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${data.cover || `https://placehold.co/80x120/1a1a1a/ffffff?text=${encodeURIComponent(data.title || 'Novel')}`})` }} />
            </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-4 text-xs text-muted-foreground">
                ...
                <span>{formatCount(data.words)} words</span>
                <span>{formatCount(data.views)} views</span>
                <span>{formatCount(data.collections)} collections</span>
              </div>
            </div>
