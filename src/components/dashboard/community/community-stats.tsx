...
export function CommunityStatsComponent({ stats = defaultStats }: CommunityStatsProps) {
  const statItems = [
    {
      icon: Users,
      value: Number(stats?.activeMembers ?? 0).toLocaleString(),
      label: "Active Members",
      color: "text-primary",
    },
    {
      icon: MessageSquare,
      value: Number(stats?.totalPosts ?? 0).toLocaleString(),
      label: "Total Posts",
      color: "text-green-500",
    },
    {
      icon: TrendingUp,
      value: Number(stats?.postsToday ?? 0),
      label: "Posts Today",
      color: "text-amber-500",
    },
    {
      icon: Award,
      value: Number(stats?.solvedQuestions ?? 0),
