...
import { ChevronLeft, ChevronRight, HelpCircle, Home, Lightbulb, MessageSquare, PenLine, Settings, Users, X } from "lucide-react"
      ...
      >
        ...
        <ScrollArea className="flex-1 py-4">
          <nav className="grid gap-2 px-2">
            ...
            <SidebarLink href="/dashboard" icon={Home} label="Overview" isOpen={isOpen || isMobile} active={pathname === "/dashboard"} />
            <SidebarLink
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              ...
              >
                <div className="relative">
                  {user?.avatar ? (
                    <img
                      src={user.avatar}
                      alt={user?.name || 'User'}
                      className="h-8 w-8 rounded-full object-cover bg-gradient-to-br from-primary to-purple-500"
                      onError={(e) => {
                        ...
                        if (parent) {
                          ...
                          fallback.className = 'h-8 w-8 rounded-full bg-gradient-to-br from-primary to-purple-500 flex items-center justify-center'
                          fallback.innerHTML = `<span class="text-sm font-medium text-white">${user?.name?.charAt(0).toUpperCase() || 'A'}</span>`
                ...
                {(isOpen || isMobile) && (
                  <div>
                    <div className="text-sm font-medium">{user?.name || 'Author'}</div>
                    <div className="text-xs text-muted-foreground">{user?.role || 'author'}</div>
                  </div>
                )}
              </div>
            </DropdownMenuTrigger>
