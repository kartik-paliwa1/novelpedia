...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Use the API service to register with Django backend
      await api.register({
        name: form.name,
        email: form.email,
        dob: form.dob,
      ...
      const message = err instanceof ApiError ? err.message : (err as Error)?.message || 'Signup failed.';
    ...
    <div className="min-h-screen flex flex-col py-10 items-center justify-center bg-gradient-to-br from-[#2d004d] via-[#4b166c] to-[#6d28d9]">
      ...
      <div className="w-[90vw] max-w-md bg-[#231942]/80 rounded-2xl shadow-xl px-4 md:px-8 py-10 flex flex-col items-center">
        ...
        <div className="mt-6 text-violet-300 text-sm text-center">
          ...
          <Link href="/login" className="text-white underline hover:text-violet-200">
            Sign in here
          </Link>
        </div>
      </div>
